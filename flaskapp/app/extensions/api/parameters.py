    ...
    def perform_patch(cls, operations, obj, state=None):
        ...
        for operation in operations:
            if not cls._process_patch_operation(operation, obj=obj, state=state):
                logger.info("%s patching has stopped because of unknown operation %s",
...
class ReturnAll(Parameters):
    return_all = base_fields.Boolean(
        decription='If return_all=true method will return all the values.', missing=False)
